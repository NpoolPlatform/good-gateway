...
func (s *Server) CreateAppPowerRental(ctx context.Context, in *npool.CreateAppPowerRentalRequest) (*npool.CreateAppPowerRentalResponse, error) {
	handler, err := powerrental1.NewHandler(
		...
		powerrental1.WithMinOrderDuration(in.MinOrderDuration, false),
		powerrental1.WithMaxOrderDuration(in.MaxOrderDuration, false),
	...
	if err != nil {
		logger.Sugar().Errorw(
			"CreateAppPowerRental",
			"In", in,
			"Error", err,
		)
		return &npool.CreateAppPowerRentalResponse{}, status.Error(codes.Aborted, err.Error())
	}
	...
	return &npool.CreateAppPowerRentalResponse{
		Info: info,
	}, nil
}

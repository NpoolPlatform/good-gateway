    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists good_manager;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name good_manager
            cd -
          done
        '''.stripIndent())
      }
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep good-gateway | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/good-gateway:latest/good-gateway:$branch/g" cmd/good-gateway/k8s/02-good-gateway.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/good-gateway/k8s/02-good-gateway.yaml
          if [ "x$REPLICAS_COUNT" == "x" ];then
            REPLICAS_COUNT=2
          fi
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/good-gateway/k8s/02-good-gateway.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/good-gateway/k8s/02-good-gateway.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/good-gateway/k8s/02-good-gateway.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
